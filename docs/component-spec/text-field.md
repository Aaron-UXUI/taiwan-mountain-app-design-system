...
- Sizes (S/M/L/XL) do not change behavior, only density.
...
| Axis | Values | Purpose |
|---|---|---|
| Size | S / M / L / XL | Matches field density to the surrounding form's scale. |
...
| Typography | `typography.body-l` / `body-m` / `body-s` depending on size, `font-weight.regular` |
